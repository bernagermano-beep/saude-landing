...
const SaudeLandingPage = () => {
  ...
  return (
    <main className="saude-landing min-h-screen overflow-hidden bg-[#f7fbfc] text-[#173844]">
      ...
      <section className="overflow-hidden px-5 py-16 sm:px-8 lg:px-12"><div className="mx-auto max-w-7xl"><p className="text-center text-xs font-bold uppercase tracking-[0.18em] text-[#6d8a92]">Operadoras que podem entrar na avaliação, conforme perfil e disponibilidade</p><div className="logo-marquee mt-9"><div className="logo-track">{[...carriers, ...carriers].map((carrier, index) => <a key={`${carrier.name}-${index}`} href={carrier.url} target="_blank" rel="noreferrer" className="carrier-logo" aria-label={`Visitar site da ${carrier.name}`}><img src={carrier.logo} alt={`Logo ${carrier.name}`} width={96} height={48} loading="lazy" /><span>{carrier.name}</span></a>)}</div></div></div></section>
